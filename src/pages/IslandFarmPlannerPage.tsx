...
import { useIslandFarmPlanner } from '../hooks/useIslandFarmPlanner';
...
import type { PlacedBuilding, PathType, TreeType } from '../types/save';
...
const ISLAND_BASE   = 'farmable' as const;

export function IslandFarmPlannerPage() {
  usePageTitle('Ginger Island Farm');
  const {
    layout, gridWidth, gridHeight, zoneMap,
    canUndo, canRedo, undo, redo, loading,
  ...
  const buildContextItems = (): ContextMenuItem[] => {
    ...
    if (contextMenu.type === 'building') {
      ...
      if (!b?.isStatic) {
        items.push({ label: 'Remove', danger: true, onClick: () => { if (b) removeBuilding(b.id); } });
      }
      return items;
    }
  ...
  if (!activeSave) {
    return (
      <div className="page">
        <h1 className="page__title">Island Farm Planner</h1>
        <p className="notice">
          <Link to="/saves">Create a save profile</Link> to use the island farm planner.
        </p>
      </div>
    );
  }

  if (!gameData?.islandFarm) {
    return (
      <div className="page">
        <h1 className="page__title">Island Farm Planner</h1>
        <p className="notice">
          Island farm data is not available. Re-run the extraction script to generate it.
        </p>
      </div>
    );
  }

  // ── Interior editor (inline, replaces island farm view) ─────────────────────
  if (interiorBuilding) {
  ...
  return (
    <div className="page--farm-planner">
      <FarmSidebar
        ...
        onSeasonChange={() => { /* island has no seasons */ }}
        zones={layout.zones}
        treeDefs={treeDefs}
        toolState={toolState}
